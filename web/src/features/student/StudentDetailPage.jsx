...
export default function StudentDetailPage() {
  ...
  const { register, handleSubmit, formState: { errors, isDirty } } = useForm({
    values: student ? { fullName: student.fullName, averageScore: student.averageScore ?? '' } : {},
  });

  async function onSave(data) {
    try {
      const averageScore = data.averageScore !== '' ? Number(data.averageScore) : null;
      await updateStudent({ id, fullName: data.fullName.trim(), averageScore }).unwrap();
  ...
  async function confirmDelete() {
    try {
      ...
      navigate(cohortId ? `/cohorts/${cohortId}/students` : '/cohorts');
    } catch (err) {
  ...
  const backTo = cohortId ? `/cohorts/${cohortId}/students` : '/cohorts';

  if (isLoading) return <div className="flex justify-center py-20"><Loader2 size={22} className="animate-spin text-ink-300" /></div>;
  ...
  return (
    <div className="max-w-2xl">
      <Link to={backTo} className="inline-flex items-center gap-1.5 text-sm text-ink-500 hover:text-ink-800 mb-6 transition-colors">
      ...
      <div className="space-y-5">
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
            <div><p className="text-ink-400 text-xs mb-0.5">Student ID</p><p className="font-mono text-ink-700">{userId?.studentId ?? '—'}</p></div>
            <div><p className="text-ink-400 text-xs mb-0.5">Email</p><p className="text-ink-700">{userId?.email ?? '—'}</p></div>
            <div><p className="text-ink-400 text-xs mb-0.5">Role</p><p className="text-ink-700 capitalize">{userId?.role ?? '—'}</p></div>
            <div>
              <p className="text-ink-400 text-xs mb-0.5">Status</p>
              <Badge variant={userId?.isActive ? 'success' : 'destructive'}>{userId?.isActive ? 'Active' : 'Deactivated'}</Badge>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="text-sm">
            ...
            <p className="font-semibold text-ink-800">
              {student.averageScore != null ? student.averageScore.toFixed(1) : <span className="text-ink-300 font-normal">—</span>}
            </p>
          </CardContent>
        </Card>
            ...
            <form onSubmit={handleSubmit(onSave)} className="space-y-4" noValidate>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input id="d-name" {...register('fullName', { required: 'Required', minLength: { value: 2 } })} aria-invalid={!!errors.fullName} />
        ...
        <Card className="border-red-100">
          ...
          <CardContent className="flex flex-wrap gap-3">
            <Button variant="outline" className="border-amber-200 text-amber-700 hover:bg-amber-50" onClick={() => setResetOpen(true)}>
              <KeyRound size={15} /> Reset Password
            </Button>
            <Button variant="outline" className="border-red-200 text-danger hover:bg-red-50" onClick={() => setDeleteOpen(true)}>
              <Trash2 size={15} /> Remove from Cohort
            </Button>
          </CardContent>
        </Card>
      </div>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Remove <span className="font-semibold text-ink-800">{student.fullName}</span> from this cohort? Their user account is preserved.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={confirmDelete} disabled={deleting}>{deleting && <Loader2 size={14} className="animate-spin" />}Remove</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={resetOpen} onOpenChange={setResetOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Generate a new temporary password for <span className="font-semibold text-ink-800">{student.fullName}</span>?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
      ...
      <Dialog open={!!resetResult} onOpenChange={(v) => !v && setResetResult(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Copy this temporary password now — not shown again.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <p className="text-sm text-ink-600"><span className="font-medium">{resetResult?.fullName}</span>{' · '}<span className="font-mono text-xs text-ink-400">{resetResult?.studentId}</span></p>
